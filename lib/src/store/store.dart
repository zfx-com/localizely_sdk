...
class Store {
  ...
  static Future<ReleaseData?> getReleaseData(String distributionId) async {
    ...
    try {
      ...
      _logger.w('Failed to load cached translations.', e);
    }
  ...
  static Future<void> persistReleaseData(
      String distributionId, ReleaseData releaseData) async {
    try {
      var platform = Platform();

      var persistedReleaseData =
          PersistedReleaseData(distributionId, releaseData);
      ...
      _logger.w('Failed to cache translations.', e);
    }
  }
  ...
  static Future<void> removePersistedReleaseData() async {
    try {
      ...
      _logger.w('Failed to clear cached translations.', e);
    }
  }
}

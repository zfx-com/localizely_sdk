...
import 'package:localizely_sdk/src/model/label.dart';
...
void main() {
  ...
  group('Label instantiation', () {
    test('Test instantiation with constructor when named args are not passed',
        () {
      var label = Label();
    ...
    test('Test instantiation with named constructor when json arg is empty',
        () {
    ...
    test('Test instantiation with named constructor when json arg has all data',
        () {
    ...
    test(
        ...
        () {
      var label = Label.fromJson(
          {'key': 'key', 'value': 'value', 'newData': 'new data'});
  ...
  group('Label literal translation', () {
    ...
    test(
        ...
        () {
      var label = Label(
          key: 'labelKey', value: 'chars: !@#\$%^&*()_+=-\\[];~`,.<>?:"\'');

      expect(label.getTranslation({}),
          equals('chars: !@#\$%^&*()_+=-\\[];~`,.<>?:"\''));
    });
    ...
    test('Test label literal translation when value contains a tag', () {
      var label =
          Label(key: 'key', value: 'Literal message with a <b>tag</b>.');
    ...
    test(
        ...
        () {
      var label = Label(
          key: 'key', value: '<i>Literal message</i> with a <br> <b>tag</b>.');
    ...
    test(
        ...
        () {
      var label =
          Label(key: 'key', value: '<p>Literal message with a <b>tag</b>.</p>');

      expect(label.getTranslation({}),
          equals('<p>Literal message with a <b>tag</b>.</p>'));
    });

    test('Test label literal translation when value contains a less-than sign',
        () {
    ...
    test(
        ...
        () {
      var label = Label(
          key: 'key', value: '{ "firstName": "John", "lastName": "Doe" }');
  ...
  group('Label argument translation', () {
    test('Test label argument translation when value contains just placeholder',
        () {
      var label = Label(key: 'key', value: '{placeholder}');

      expect(label.getTranslation({'placeholder': 'Some value'}),
          equals('Some value'));
    });
    ...
    test(
        ...
        () {
      var label = Label(
          key: 'key', value: 'My name is {lastName}, {firstName} {lastName}!');

      expect(label.getTranslation({'lastName': 'Bond', 'firstName': 'James'}),
    ...
    test(
        ...
        () {
      var label = Label(
          key: 'key', value: 'Argument message with {placeholder} and < sign');
  ...
  group('Label plural translation', () {
    test('Test label plural translation when value contains empty plural forms',
        () {
      var label = Label(
          key: 'labelKey',
          value:
    ...
    test(
      ...
      expect(label.getTranslation({'howMany': 1, 'placeholder': 'Some value'}),
          equals('one-pf: Some value'));
      expect(label.getTranslation({'howMany': 2, 'placeholder': 'Some value'}),
    ...
    test(
      ...
      expect(
          label.getTranslation(
              {'howMany': 1, 'first': 'first-val', 'second': 'second-val'}),
          equals('one-pf: 1 first-val second-val'));
      expect(
          label.getTranslation(
              {'howMany': 2, 'first': 'first-val', 'second': 'second-val'}),
    ...
    test(
      ...
      expect(label.getTranslation({'howMany': 1, 'first': 'first-val'}),
          equals('one-pf-chars: !@#\$%^&*()_+=-\\[];~`,.>?:"\' 1 first-val'));
      expect(label.getTranslation({'howMany': 2, 'first': 'first-val'}),
    ...
    test(
      ...
      expect(
          label.getTranslation({'howMany': 1}), equals('one-pf: <b>tag</b>'));
      expect(
          label.getTranslation({'howMany': 2}), equals('other-pf: <b>tag</b>'));
    }, skip: true);
    ...
    test(
      ...
      expect(label.getTranslation({'count': 1}),
          equals('one message { "firstName": "John", "lastName": "Doe" }'));
      expect(label.getTranslation({'count': 5}),
          equals('other message { "firstName": "John", "lastName": "Doe" }'));
    }, skip: true);
  });

  group('Label gender translation', () {
    test('Test label gender translation when value contains empty gender forms',
        () {
      var label = Label(
          key: 'key', value: '{gender, select, male {} female {} other {}}');
    ...
    test(
      ...
      expect(
          label.getTranslation(
              {'gender': 'female', 'placeholder': 'Some value'}),
          equals('female-gf: Some value'));
      expect(
          label
              .getTranslation({'gender': 'other', 'placeholder': 'Some value'}),
    ...
    test(
      ...
      expect(
          label.getTranslation(
              {'gender': 'male', 'first': 'first-val', 'second': 'second-val'}),
    ...
    test(
      ...
      expect(label.getTranslation({'gender': 'male'}),
          equals('male-gf: <b>tag</b>'));
      expect(label.getTranslation({'gender': 'female'}),
          equals('female-gf: <b>tag</b>'));
      expect(label.getTranslation({'gender': 'other'}),
    ...
    test(
      ...
      expect(label.getTranslation({'gender': 'male'}), equals('male-gf: <'));
      expect(
          label.getTranslation({'gender': 'female'}), equals('female-gf: <'));
      expect(label.getTranslation({'gender': 'other'}), equals('other-gf: <'));
    }, skip: true);
  ...
  group('Label select translation', () {
    test('Test label select translation when value contains empty select forms',
        () {
      var label = Label(
          key: 'key', value: '{choice, select, foo {} bar {} baz {} other{}}');
    ...
    test(
      ...
      expect(
          label.getTranslation(
              {'choice': 'foo', 'first': 'first-val', 'second': 'second-val'}),
          equals('foo-sf: foo first-val second-val'));
      expect(
          label.getTranslation(
              {'choice': 'bar', 'first': 'first-val', 'second': 'second-val'}),
          equals('bar-sf: bar first-val second-val'));
      expect(
          label.getTranslation(
              {'choice': 'baz', 'first': 'first-val', 'second': 'second-val'}),
          equals('baz-sf: baz first-val second-val'));
      expect(
          label.getTranslation(
              {'choice': 'ups', 'first': 'first-val', 'second': 'second-val'}),
    ...
    test(
      ...
      // `<` char is not used in the test - cause parsing error
      expect(label.getTranslation({'choice': 'foo', 'first': 'first-val'}),
          equals('foo-sf-chars: !@#\$%^&*()_+=-\\[];~`,.>?:"\' foo first-val'));
      expect(label.getTranslation({'choice': 'bar', 'first': 'first-val'}),
          equals('bar-sf-chars: !@#\$%^&*()_+=-\\[];~`,.>?:"\' bar first-val'));
      expect(label.getTranslation({'choice': 'baz', 'first': 'first-val'}),
    ...
    test(
      ...
      expect(label.getTranslation({'choice': 'foo'}),
          equals('foo-sf: <b>tag</b>'));
      expect(label.getTranslation({'choice': 'bar'}),
          equals('bar-sf: <b>tag</b>'));
      expect(label.getTranslation({'choice': 'baz'}),
          equals('baz-sf: <b>tag</b>'));
      expect(label.getTranslation({'choice': 'ups'}),
    ...
    test(
      ...
      expect(label.getTranslation({'choice': 'other'}),
          equals('other message { "firstName": "John", "lastName": "Doe" }'));
    }, skip: true);
  });
  ...
  group('Label compound translation', () {
    test(
      ...
      expect(label.getTranslation({'count': 5}), equals('John has 5 apples.'));
    });
    ...
    test(
      ...
      expect(label.getTranslation({'gender': 'male', 'name': 'John'}),
          equals('Welcome Mr John.'));
      expect(label.getTranslation({'gender': 'female', 'name': 'Jane'}),
          equals('Welcome Mrs Jane.'));
      expect(label.getTranslation({'gender': 'other', 'name': 'Alex'}),
    ...
    test(
      ...
      expect(label.getTranslation({'choice': 'admin', 'name': 'Alex'}),
          equals('The admin Alex.'));
      expect(label.getTranslation({'choice': 'owner', 'name': 'Alex'}),
          equals('The owner Alex.'));
      expect(label.getTranslation({'choice': 'other', 'name': 'Alex'}),
    ...
    test(
      ...
      expect(label.getTranslation({'name': 'Alex', 'count': 1}),
          equals('Alex has 1 apple in the bag.'));
      expect(label.getTranslation({'name': 'Alex', 'count': 5}),
    ...
    test(
      ...
      expect(
          label.getTranslation(
              {'gender': 'male', 'name': 'John', 'device': 'radio'}),
          equals('The Mr John has the radio.'));
      expect(
          label.getTranslation(
              {'gender': 'female', 'name': 'Jane', 'device': 'radio'}),
          equals('The Mrs Jane has the radio.'));
      expect(
          label.getTranslation(
              {'gender': 'other', 'name': 'Alex', 'device': 'radio'}),
    ...
    test(
      ...
      expect(
          label.getTranslation(
              {'choice': 'coffee', 'name': 'espresso', 'client': 'Alex'}),
          equals('The one espresso coffee please for the Alex.'));
      expect(
          label.getTranslation(
              {'choice': 'tea', 'name': 'green', 'client': 'Alex'}),
          equals('The one green tea please for the Alex.'));
      expect(
          label.getTranslation(
              {'choice': 'other', 'name': 'juice', 'client': 'Alex'}),
    ...
    test(
      ...
      expect(label.getTranslation({'count1': 1, 'count2': 2}),
          equals('p1 one.p2 other.'));
      expect(label.getTranslation({'count1': 0, 'count2': 0}),
          equals('p1 other.p2 other.'));
      expect(label.getTranslation({'count1': 5, 'count2': 1}),
    ...
    test('Test label compound translation when value contains a less-than sign',
        () {
      var label = Label(
          key: 'key',
          value:
      ...
      expect(
          label
              .getTranslation({'gender': 'female', 'name': 'Jane', 'count': 1}),
          equals('Mrs Jane has < 1 apple.'));
      expect(
          label
              .getTranslation({'gender': 'bla', 'name': 'Unknown', 'count': 0}),
